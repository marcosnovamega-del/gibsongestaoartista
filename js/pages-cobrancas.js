...
Pages.renderCobrancas = async function () {
    ...
    const enriched = parcelas.map(p => {
        ...
        const venc = p.data_vencimento ? new Date(p.data_vencimento + 'T12:00:00') : null;
...
Pages._cobRenderTabela = function (dados, filtro, busca) {
    ...
    const linhas = lista.map(p => {
        ...
        const dataPagoHtml = p._estado === 'paga' && p.data_pagamento
            ? `<div style="font-size:11px;color:var(--success);margin-top:2px;"><i class="fas fa-check"></i> Pago em ${Utils.formatDate(p.data_pagamento)}</div>`
            : '';
