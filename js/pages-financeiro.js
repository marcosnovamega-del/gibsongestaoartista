    ...
    if (Auth.currentUser?.nivel === 'Vendedor') {
        document.getElementById('pageContent').innerHTML = `
            <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:60vh;gap:16px;color:var(--text-muted);">
                <i class="fas fa-lock" style="font-size:48px;opacity:0.4;"></i>
                <h3 style="margin:0;">Acesso Restrito</h3>
                <p style="margin:0;">Esta área não está disponível para vendedores.</p>
            </div>`;
        return;
    }
    const hoje = new Date();
...
Pages.carregarRecebimentosAConfirmar = async function() {
    ...
    try {
        ...
        // Barra de controles: filtro de cidade (dropdown) + botões PDF
        // Chips ocultos mantidos para que _exportarRecebimentosPDF leia .rec-city-chip.active
        const cidadeChips = `
        <div style="display:none" aria-hidden="true">
            <button class="rec-city-chip active" data-city="">Todas</button>
            ${cidades.map(c => `<button class="rec-city-chip" data-city="${c}">${c}</button>`).join('')}
        </div>
        <div class="rec-controls" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;margin-bottom:20px;">
            <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
                <span style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:var(--text-secondary);white-space:nowrap;">
                    <i class="fas fa-map-marker-alt" style="color:#D4AF37;margin-right:4px;"></i> Filtrar cidade:
                </span>
                <div style="position:relative;">
                    <select id="rec-city-select"
                        onchange="var v=this.value; Pages._filtrarRecebimentos(v||null,null); document.querySelectorAll('.rec-city-chip').forEach(function(b){b.classList.toggle('active',b.dataset.city===v);});"
                        style="appearance:none;-webkit-appearance:none;padding:7px 36px 7px 14px;border-radius:10px;border:1.5px solid var(--border-color);background:var(--bg-secondary);color:var(--text-primary);font-size:13px;font-weight:600;cursor:pointer;outline:none;min-width:200px;transition:border-color .15s;"
                        onfocus="this.style.borderColor='#D4AF37'" onblur="this.style.borderColor='var(--border-color)'">
                        <option value="">Todas as cidades (${cidades.length})</option>
                        ${cidades.map(c => `<option value="${c}">${c}</option>`).join('')}
                    </select>
                    <i class="fas fa-chevron-down" style="position:absolute;right:12px;top:50%;transform:translateY(-50%);pointer-events:none;color:#D4AF37;font-size:11px;"></i>
                </div>
            </div>
            <button onclick="Pages._exportarRecebimentosPDF()"
                style="background:linear-gradient(135deg,#c0392b,#e74c3c);color:#fff;border:none;border-radius:8px;padding:8px 16px;font-size:12px;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:6px;white-space:nowrap;">
...
Pages._pagarComissao = function(despesaId, paginaCallback) {
    var hoje = new Date().toLocaleDateString('fr-CA');
...
Pages._agendarComissao = function(despesaId, paginaCallback) {
    ...
    var defaultDate = proxSemana.toLocaleDateString('fr-CA');
                                ...
                                <td>
                                    ...
                                    ` : p.status === 'Pago' ? `
                                        <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
                                            <small class="text-muted"><i class="fas fa-check-circle" style="color:var(--success);"></i> ${p.data_pagamento ? new Date(String(p.data_pagamento).slice(0,10)+'T12:00:00').toLocaleDateString('pt-BR') : 'Pago'}</small>
...
Pages.marcarParcelaPaga = async function(parcelaId) {
    if (Auth.currentUser?.nivel === 'Vendedor') {
        Utils.showToast('Acesso restrito. Vendedores não podem marcar pagamentos.', 'error');
        return;
    }
...
Pages.renderAlertas = async function() {
    ...
    const html = `
        <div class="alertas-container">
            ...
            <!-- Despesas Pendentes (oculto para Vendedor) -->
            ${despesasPendentes.length > 0 && Auth.currentUser?.nivel !== 'Vendedor' ? `
                <div class="card mb-3" style="border-left: 4px solid var(--warning);">
                    <div class="card-header" style="background: rgba(245, 158, 11, 0.1);">
                        <h3 class="card-title" style="color: var(--warning);">
        ...
        html += `
            <div style="padding: 12px; border-bottom: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: center;">
                ...
                <div style="text-align: right;">
                    ...
                    ${Auth.currentUser?.nivel !== 'Vendedor' ? `<button class="btn-primary btn-sm mt-1" onclick="Pages.marcarParcelaPaga('${parcela.id}')">Marcar Pago</button>` : ''}
                </div>
            </div>
        `;
    }
    ...
    for (const contrato of contratos) {
        ...
        html += `
            <div style="padding: 12px; border-bottom: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: center;">
                ...
                <button class="btn-primary btn-sm" onclick="Pages.assinarContratoModal('', '')">
                    <i class="fas fa-signature"></i> Ver Contrato
                </button>
            </div>
        `;
    }
