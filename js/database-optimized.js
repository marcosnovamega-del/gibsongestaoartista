...
const PropostasDB = {
    ...
    async converterParaEvento(propostaId) {
        ...
        if (evento) {
            ...
            // Liberar datas alternativas E reservas orphans (data primária que não foi a escolhida)
            try {
                const todosEventos = await EventosDB.listar(true);
                // Deleta Reserva Alt. e também qualquer Reserva da data primária que ficou orphan
                // (caso o evento tenha sido confirmado para uma data alternativa diferente da primária)
                const paraLimpar = todosEventos.filter(e =>
                    e.artista_id === proposta.artista_id && (
                        e.status === 'Reserva Alt.' ||
                        (e.status === 'Reserva' && e.data === proposta.data_evento && e.id !== evento.id)
                    )
                );
                for (const ev of paraLimpar) {
                    await DB.delete('eventos', ev.id);
                }
                if (paraLimpar.length > 0) {
                    console.log(`[converterParaEvento] ${paraLimpar.length} reserva(s) orphan liberada(s).`);
                }
            } catch (err) {
...
const TurnesDB = {
    ...
    async listarMensagens(turneId) {
        ...
        return msgs.map(m => ({
            ...m,
            usuario_nome: (usuarios.find(u => u.id === m.usuario_id) || {}).nome || 'Usuário'
        }));
    },
};