...
Pages.marcarReceitaRecebida = async function(id) {
    const r = await ReceitasDB.atualizar(id, { status: 'Recebido', data_recebimento: new Date().toISOString().split('T')[0] });
...
Pages.marcarDespesaPaga = async function(id) {
    const r = await DespesasDB.atualizar(id, { status: 'Pago', data_pagamento: new Date().toISOString().split('T')[0] });
