...
const Pages = {
    ...
    async renderDashboard() {
        ...
        const [todosEventos, parcelas, todasDespesas, todasPropostas] = await Promise.all([
            EventosDB.listar(),
            ParcelasDB.listar(),
            DespesasDB.listar(),
            isVendedor ? PropostasDB.listar() : Promise.resolve([])
        ]);

        // Filtrar por artista selecionado
        let eventos = artistaVinculadoId
            ? todosEventos.filter(e => e.artista_id === artistaVinculadoId)
            : todosEventos;

        // Vendedor: mostrar apenas eventos das SUAS propostas
        if (isVendedor) {
            const meuNome = Auth.currentUser.nome;
            const minhasPropostasIds = new Set(
                todasPropostas
                    .filter(p => p.vendedor_nome === meuNome)
                    .map(p => p.id)
            );
            eventos = eventos.filter(e => e.proposta_id && minhasPropostasIds.has(e.proposta_id));
        }
        ...
        const parcelasAtrasadas = parcelas.filter(p => {
            ...
            if (new Date(String(p.data_vencimento).slice(0,10)+'T12:00:00') >= hoje) return false;
    ...
    async renderArtistas(isSearch = false) {
        ...
        const maxComissao = artistas.reduce((m, a) => Math.max(m, a.comissao_padrao || 0), 0) || 1;
        const ativos = artistas.filter(a => a.status === 'Ativo').length;
        ...
        const html = `
            <div class="fade-in" style="padding:0;">
                ...
                <div style="display:flex;align-items:flex-end;justify-content:space-between;margin-bottom:20px">
                    <div>
                        <div style="font-size:11px;letter-spacing:2.5px;font-weight:700;color:var(--brand-primary);margin-bottom:8px">ELENCO</div>
                        <h1 style="font-size:26px;font-weight:800;letter-spacing:-.5px;margin:0">Artistas</h1>
                        <p style="font-size:13px;color:var(--text-muted);margin-top:5px;font-weight:500">${artistas.length} cadastrado(s) · ${ativos} ativo(s)${this.currentSearchTerm ? ' · busca: <strong>"' + this.currentSearchTerm + '"</strong>' : ''}</p>
                    </div>
                    <div style="display:flex;gap:9px">
                        <button class="btn-secondary" onclick="Utils.exportToExcel(artistas, 'lista_artistas')"><i class="fas fa-file-excel"></i> Exportar</button>
                        ${Auth.isAdmin() ? `<button class="btn-primary" onclick="Modals.showArtistaModal()"><i class="fas fa-plus"></i> Novo artista</button>` : ''}
                    </div>
                </div>
                ...
                ${artistas.length > 0 ? `
                    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:16px">
                        ${artistas.map((artista, idx) => {
                            const iniciais = artista.nome.split(' ').slice(0,2).map(w=>w[0]).join('').toUpperCase();
                            const isAtivo = artista.status === 'Ativo';
                            const comissaoPct = maxComissao > 0 ? ((artista.comissao_padrao || 0) / maxComissao * 100).toFixed(0) : 0;
                            const rankCor = idx === 0 ? 'var(--brand-primary)' : 'var(--text-muted)';
                            const rankBg = idx === 0 ? 'rgba(244,196,48,0.1)' : 'var(--bg-secondary)';
                            return `
                            <div style="background:var(--card-bg);border:1px solid var(--border-color);border-radius:15px;padding:18px;position:relative">
                                <div style="position:absolute;top:16px;right:16px;display:flex;align-items:center;gap:5px;background:${rankBg};color:${rankCor};font-size:11px;font-weight:700;padding:3px 8px;border-radius:7px">${idx === 0 ? '★ ' : ''}#${idx + 1}</div>

                                <div style="display:flex;align-items:center;gap:13px">
                                    <div style="width:52px;height:52px;border-radius:13px;background:var(--bg-secondary);border:1px solid var(--border-color);display:flex;align-items:center;justify-content:center;font-weight:800;font-size:17px;color:${idx===0?'var(--brand-primary)':'var(--text-secondary)'};flex-shrink:0">${iniciais}</div>
                                    <div style="min-width:0">
                                        <div style="font-size:15.5px;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${artista.nome}</div>
                                        <div style="margin-top:6px;display:inline-flex;align-items:center;gap:5px;background:${isAtivo?'rgba(78,201,138,0.12)':'var(--bg-secondary)'};color:${isAtivo?'var(--success)':'var(--text-muted)'};font-size:10px;font-weight:700;padding:2px 8px;border-radius:6px">● ${isAtivo?'ATIVO':'INATIVO'}</div>
                                    </div>
                                </div>

                                <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1px;margin-top:16px;background:var(--border-color);border-radius:11px;overflow:hidden">
                                    <div style="background:var(--card-bg);padding:11px 10px;text-align:center">
                                        <div style="font-size:9.5px;letter-spacing:.5px;color:var(--text-muted);font-weight:600">FATURAM.</div>
                                        <div style="font-size:13.5px;font-weight:700;color:var(--text-muted);margin-top:4px">—</div>
                                    </div>
                                    <div style="background:var(--card-bg);padding:11px 10px;text-align:center">
                                        <div style="font-size:9.5px;letter-spacing:.5px;color:var(--text-muted);font-weight:600">SHOWS/MÊS</div>
                                        <div style="font-size:13.5px;font-weight:700;color:var(--text-muted);margin-top:4px">—</div>
                                    </div>
                                    <div style="background:var(--card-bg);padding:11px 10px;text-align:center">
                                        <div style="font-size:9.5px;letter-spacing:.5px;color:var(--text-muted);font-weight:600">PRÓXIMO</div>
                                        <div style="font-size:13.5px;font-weight:700;color:var(--text-muted);margin-top:4px">—</div>
                                    </div>
                                </div>
                                ...
                                <div style="margin-top:14px">
                                    <div style="display:flex;justify-content:space-between;font-size:11px;color:var(--text-muted)">
                                        <span>Comissão do escritório</span>
                                        <span style="font-weight:600">${Utils.formatCurrency(artista.comissao_padrao || 0)}</span>
                                    </div>
                                    <div style="height:5px;border-radius:3px;background:var(--border-color);margin-top:7px;overflow:hidden">
                                        <div style="width:${comissaoPct}%;height:100%;background:${idx===0?'var(--brand-primary)':'var(--text-muted)'};border-radius:3px"></div>
                                    </div>
                                </div>

                                <div style="display:flex;gap:8px;margin-top:15px">
                                    <button class="btn-secondary" style="flex:1;padding:8px;font-size:12px;border-radius:8px" onclick="Pages.renderArtistaProfile('${artista.id}')">Ver perfil</button>
                                    <button class="btn-secondary" style="flex:1;padding:8px;font-size:12px;border-radius:8px" onclick="Pages.changePage('turnes')">Agenda</button>
                                </div>
                            </div>`;
                        }).join('')}

                        ${Auth.isAdmin() ? `
                        <div style="border:1.5px dashed var(--border-color);border-radius:15px;padding:18px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;min-height:200px;cursor:pointer;color:var(--text-muted)" onclick="Modals.showArtistaModal()">
                            <div style="width:44px;height:44px;border-radius:12px;background:var(--bg-secondary);border:1px solid var(--border-color);display:flex;align-items:center;justify-content:center;font-size:22px;color:var(--brand-primary)">+</div>
                            <div style="font-size:13.5px;font-weight:700">Cadastrar artista</div>
                            <div style="font-size:11.5px;text-align:center;max-width:170px">Adicione um novo artista ao elenco do escritório</div>
                        </div>` : ''}
                    </div>
                ` : `
                    <div style="background:var(--card-bg);border:1px solid var(--border-color);border-radius:15px;padding:60px;text-align:center">
                        <i class="fas fa-microphone" style="font-size:48px;color:var(--text-muted);margin-bottom:16px;display:block;opacity:.4"></i>
                        <h3 style="color:var(--text-secondary);font-weight:700">Nenhum artista cadastrado</h3>
                        ${Auth.isAdmin() ? `<button class="btn-primary mt-2" onclick="Modals.showArtistaModal()"><i class="fas fa-plus"></i> Cadastrar Primeiro Artista</button>` : ''}
                    </div>
                `}
            </div>
        `;
