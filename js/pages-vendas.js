    ...
    return `
        <div class="kanban-card ${isFechado ? 'kanban-card-fechado' : ''}" ${isExpirada ? 'style="opacity:0.75;"' : ''}>
            ...
            <div class="kanban-card-actions">
                ...
                ` : ''}
                ${coluna === 3 && Auth.currentUser?.nivel !== 'Vendedor' ? `
...
Pages.aceitarProposta = async function(id) {
    ...
    if (!ok) return;
    Utils.showLoading();
    // Pré-correção: garantir campos críticos antes de converter (evita erro de constraint no banco)
    try {
        const proposta = await PropostasDB.buscarPorId(id);
        if (proposta) {
            const fixes = {};
            if (!proposta.horario || proposta.horario === '') fixes.horario = '00:00:00';
            if (!proposta.estado_evento || proposta.estado_evento === '') fixes.estado_evento = '-';
            if (!proposta.local_evento || proposta.local_evento === '') fixes.local_evento = proposta.cidade_evento || 'A definir';
            if (!proposta.cidade_evento || proposta.cidade_evento === '') fixes.cidade_evento = 'A definir';
            if (Object.keys(fixes).length > 0) await PropostasDB.atualizar(id, fixes);
        }
    } catch(e) { console.warn('[aceitarProposta] pre-fix:', e); }
...
Pages.assinarContratoModal = async function(eventoId, propostaId) {
    if (Auth.currentUser?.nivel === 'Vendedor') {
        Utils.showToast('Acesso restrito. Vendedores não podem visualizar contratos.', 'error');
        return;
    }
    Utils.showLoading();
            ...
            <table style="width:100%;border-collapse:collapse;font-size:13px;">
                ...
                <tbody>
                    ${cronograma.map((item, i) => {
                        ...
                        let vencText = '—';
                        let vencDateStr = null;
                        if (item.data_vencimento) {
                            vencDateStr = String(item.data_vencimento).slice(0,10);
                            vencText = Utils.formatDate(vencDateStr);
                            ...
                            vencDateStr = venc.toISOString().split('T')[0];
                            vencText = Utils.formatDate(vencDateStr);
                        }
                        const hoje0 = new Date(); hoje0.setHours(0,0,0,0);
                        const vencD = vencDateStr ? new Date(vencDateStr + 'T12:00:00') : null;
                        const atrasada = vencD && vencD < hoje0;
                        const vencStyle = atrasada
                            ? 'color:#e8261c;font-weight:700;'
                            : 'color:var(--text-primary);';
                        const vencLabel = atrasada
                            ? `${vencText} <span style="font-size:10px;background:#fee2e2;color:#c0392b;border-radius:4px;padding:1px 5px;margin-left:4px;font-weight:700;">VENCIDA</span>`
                            : vencText;
                        return `<tr style="border-bottom:1px solid var(--border-color);${atrasada ? 'background:rgba(232,38,28,0.04);' : ''}">
                            ...
                            <td style="padding:6px 8px;${vencStyle}">${vencLabel}</td>
                        </tr>`;
                    }).join('')}
                </tbody>
                <tfoot>
                    ...
                    <div id="crono-${p.id}" style="display:none;border-top:1px solid var(--border-color);padding:16px;">
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;flex-wrap:wrap;gap:8px;">
                            ...
                            <div style="display:flex;gap:6px;flex-wrap:wrap;">
                                ...
                                ` : ''}
                                ${p.status === 'Aceita' && p.contrato && p.contrato.status !== 'Assinado' && Auth.currentUser?.nivel !== 'Vendedor' ? `
...
Pages.renderComissaoVendedor = async function () {
    ...
    minhas.forEach(function(d) {
        ...
        var dtPago    = d.data_pagamento  ? new Date(String(d.data_pagamento).slice(0,10)+'T12:00:00').toLocaleDateString('pt-BR') : '';
    ...
    pc.innerHTML = '<div class="fade-in">' +
        ...
        '<div class="stat-card"><div class="stat-icon" style="background:rgba(212,175,55,0.15)"><i class="fas fa-hand-holding-usd" style="color:var(--brand-primary)"></i></div><div class="stat-content"><h3>' + Utils.formatCurrency(total) + '</h3><p>Total Geral</p></div></div>' +
        '</div>' +
