...
Pages.renderVeiculos = async function () {
    ...
    try {
        ...
        <div id="modalViagem" class="modal-overlay" style="display:none;" onclick="if(event.target===this)Pages._fecharModalViagem()">
            <div class="modal-container" style="max-width:780px;width:95%;background:#fff;border-radius:16px;overflow:hidden;">
                <div class="modal-header" style="background:#1a202c;border-bottom:none;">
                    <h3 id="modalViagemTitulo" style="color:#fff;"><i class="fas fa-route" style="color:#c9a227;margin-right:8px;"></i> Nova Viagem</h3>
                    <button class="modal-close" onclick="Pages._fecharModalViagem()" style="color:#fff;background:rgba(255,255,255,0.1);border-radius:8px;border:none;width:32px;height:32px;cursor:pointer;font-size:16px;"><i class="fas fa-times"></i></button>
                </div>
                <div class="modal-body" id="modalViagemBody" style="max-height:80vh;overflow-y:auto;background:#fff;padding:24px;"></div>
            </div>
        </div>

        <!-- Modal Veículo -->
        <div id="modalVeiculo" class="modal-overlay" style="display:none;" onclick="if(event.target===this)Pages._fecharModalVeiculo()">
            <div class="modal-container" style="max-width:500px;width:95%;background:#fff;border-radius:16px;overflow:hidden;">
                <div class="modal-header" style="background:#1a202c;border-bottom:none;">
                    <h3 id="modalVeiculoTitulo" style="color:#fff;"><i class="fas fa-bus" style="color:#c9a227;margin-right:8px;"></i> Cadastrar Veículo</h3>
                    <button class="modal-close" onclick="Pages._fecharModalVeiculo()" style="color:#fff;background:rgba(255,255,255,0.1);border-radius:8px;border:none;width:32px;height:32px;cursor:pointer;font-size:16px;"><i class="fas fa-times"></i></button>
                </div>
                <div class="modal-body" id="modalVeiculoBody" style="max-height:80vh;overflow-y:auto;background:#fff;padding:24px;"></div>
            </div>
        </div>
        ...
        <style>
            ...
            /* ── Modais de veículos — tema branco ── */
            #modalViagem .modal-body,
            #modalVeiculo .modal-body {
    ...
    <form id="formViagem" onsubmit="event.preventDefault(); Pages._salvarViagem();">
        <div class="form-grid" style="grid-template-columns:1fr 1fr; gap:14px; margin-bottom:18px;">
            <div class="form-group">
                <label style="display:block;font-size:10px;font-weight:700;color:#64748b;margin-bottom:5px;text-transform:uppercase;letter-spacing:1px;">Veículo *</label>
                <select class="form-control" name="veiculo_id" required>${optVei}</select>
            </div>
            <div class="form-group">
                <label style="display:block;font-size:10px;font-weight:700;color:#64748b;margin-bottom:5px;text-transform:uppercase;letter-spacing:1px;">Evento Vinculado</label>
                <select class="form-control" name="evento_id">${optEv}</select>
            </div>
            <div class="form-group">
                <label style="display:block;font-size:10px;font-weight:700;color:#64748b;margin-bottom:5px;text-transform:uppercase;letter-spacing:1px;">Data Início</label>
                <input type="date" class="form-control" name="data_inicio" value="${v.data_inicio || ''}">
            </div>
            <div class="form-group">
                <label style="display:block;font-size:10px;font-weight:700;color:#64748b;margin-bottom:5px;text-transform:uppercase;letter-spacing:1px;">Data Fim</label>
                <input type="date" class="form-control" name="data_fim" value="${v.data_fim || ''}">
            </div>
            <div class="form-group">
                <label style="display:block;font-size:10px;font-weight:700;color:#64748b;margin-bottom:5px;text-transform:uppercase;letter-spacing:1px;">KM Inicial (hodômetro)</label>
                <input type="number" step="0.1" class="form-control" name="km_inicial" value="${v.km_inicial || ''}" placeholder="Ex: 793713.8">
            </div>
            <div class="form-group">
                <label style="display:block;font-size:10px;font-weight:700;color:#64748b;margin-bottom:5px;text-transform:uppercase;letter-spacing:1px;">KM Final (hodômetro)</label>
                <input type="number" step="0.1" class="form-control" name="km_final" value="${v.km_final || ''}" placeholder="Ex: 795674.3">
            </div>
            <div class="form-group">
                <label style="display:block;font-size:10px;font-weight:700;color:#64748b;margin-bottom:5px;text-transform:uppercase;letter-spacing:1px;">Receita de Transporte (R$)</label>
                <input type="number" step="0.01" class="form-control" name="receita_transporte" value="${v.receita_transporte || ''}" placeholder="0,00">
            </div>
            <div class="form-group">
                <label style="display:block;font-size:10px;font-weight:700;color:#64748b;margin-bottom:5px;text-transform:uppercase;letter-spacing:1px;">Status</label>
        ...
        <div style="margin-bottom:20px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                <h4 style="margin:0; font-size:15px; color:#1a202c;"><i class="fas fa-map-marked-alt" style="color:#c9a227;margin-right:6px;"></i> Trechos da Viagem</h4>
        ...
        <div style="margin-bottom:20px;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                <h4 style="margin:0; font-size:15px; color:#1a202c;"><i class="fas fa-receipt" style="color:#c9a227;margin-right:6px;"></i> Outras Despesas</h4>
        ...
        <div class="form-group" style="margin-bottom:20px;">
            <label style="display:block;font-size:10px;font-weight:700;color:#64748b;margin-bottom:5px;text-transform:uppercase;letter-spacing:1px;">Observações</label>
...
Pages._htmlTrecho = function(idx, t = {}) {
    ...
    <div class="trecho-row" id="trecho-${idx}">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
            <span style="font-weight:700; font-size:13px; color:#1a202c;">
                <i class="fas fa-map-pin" style="color:#c9a227;margin-right:4px;"></i> Trecho ${idx + 1}
            </span>
        ...
        <div class="trecho-grid" style="margin-bottom:10px;">
            <div class="form-group" style="margin:0;">
                <label style="display:block;font-size:10px;font-weight:700;color:#64748b;margin-bottom:4px;text-transform:uppercase;letter-spacing:1px;">Data</label>
                <input type="date" class="form-control" name="tr_data_${idx}" value="${t.data || ''}">
            </div>
            <div class="form-group" style="margin:0;">
                <label style="display:block;font-size:10px;font-weight:700;color:#64748b;margin-bottom:4px;text-transform:uppercase;letter-spacing:1px;">Cidade Origem</label>
                <input type="text" class="form-control" name="tr_origem_${idx}" value="${t.cidade_origem || ''}" placeholder="Ex: Dores de Campos">
            </div>
            <div class="form-group" style="margin:0;">
                <label style="display:block;font-size:10px;font-weight:700;color:#64748b;margin-bottom:4px;text-transform:uppercase;letter-spacing:1px;">Cidade Destino</label>
        ...
        <div class="trecho-grid-4">
            <div class="form-group" style="margin:0;">
                <label style="display:block;font-size:10px;font-weight:700;color:#64748b;margin-bottom:4px;text-transform:uppercase;letter-spacing:1px;">KM Saída</label>
                <input type="number" step="0.1" class="form-control" name="tr_kmsaida_${idx}" value="${t.km_saida || ''}" placeholder="793713.8">
            </div>
            <div class="form-group" style="margin:0;">
                <label style="display:block;font-size:10px;font-weight:700;color:#64748b;margin-bottom:4px;text-transform:uppercase;letter-spacing:1px;">KM Chegada</label>
                <input type="number" step="0.1" class="form-control" name="tr_kmchegada_${idx}" value="${t.km_chegada || ''}" placeholder="794410.2">
            </div>
            <div class="form-group" style="margin:0;">
                <label style="display:block;font-size:10px;font-weight:700;color:#64748b;margin-bottom:4px;text-transform:uppercase;letter-spacing:1px;">KM Abastecimento</label>
                <input type="number" step="0.1" class="form-control" name="tr_kmabast_${idx}" value="${t.km_abastecimento || ''}" placeholder="793923.1">
            </div>
            <div class="form-group" style="margin:0;">
                <label style="display:block;font-size:10px;font-weight:700;color:#64748b;margin-bottom:4px;text-transform:uppercase;letter-spacing:1px;">Valor Abastecimento</label>
...
Pages._htmlDespExtra = function(idx, d = {}) {
    ...
    <div class="despesa-row" id="desp-${idx}">
        <div class="form-group" style="margin:0;">
            <label style="display:block;font-size:10px;font-weight:700;color:#64748b;margin-bottom:4px;text-transform:uppercase;letter-spacing:1px;">Tipo</label>
        ...
        <div class="form-group" style="margin:0;">
            <label style="display:block;font-size:10px;font-weight:700;color:#64748b;margin-bottom:4px;text-transform:uppercase;letter-spacing:1px;">Descrição</label>
            <input type="text" class="form-control" name="dp_desc_${idx}" value="${d.descricao || ''}" placeholder="Descrição">
        </div>
        <div class="form-group" style="margin:0;">
            <label style="display:block;font-size:10px;font-weight:700;color:#64748b;margin-bottom:4px;text-transform:uppercase;letter-spacing:1px;">Valor (R$)</label>
...
Pages._renderModalVeiculoBody = function(vei, editId) {
    const v = vei || {};
    const IS = 'width:100%;padding:11px 14px;background:#fff;border:1.5px solid #e2e8f0;border-radius:10px;color:#1a202c;font-size:14px;box-sizing:border-box;transition:border-color .15s;outline:none;font-family:inherit;';
    const LB = (icon, txt) => `<label style="display:flex;align-items:center;gap:6px;font-size:10px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px;"><i class="fas fa-${icon}" style="color:#c9a227;width:12px;text-align:center;"></i>${txt}</label>`;
    const FO = `onfocus="this.style.borderColor='#c9a227';this.style.boxShadow='0 0 0 3px rgba(201,162,39,0.12)'" onblur="this.style.borderColor='#e2e8f0';this.style.boxShadow='none'"`;

    document.getElementById('modalVeiculoBody').innerHTML = `
    <form id="formVeiculo" onsubmit="event.preventDefault(); Pages._salvarVeiculo();">
        <div style="background:#fff;border-radius:12px;padding:4px 0 8px;">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;padding:8px 0 20px;">

            <div style="grid-column:span 2;">
                ${LB('bus','Nome / Apelido *')}
                <input type="text" name="nome" value="${v.nome||''}" placeholder="Ex: Ônibus Principal" required style="${IS}" ${FO}>
            </div>

            <div>
                ${LB('hashtag','Placa')}
                <input type="text" name="placa" value="${v.placa||''}" placeholder="ABC-1234" style="${IS}text-transform:uppercase;" ${FO}>
            </div>

            <div>
                ${LB('car','Modelo')}
                <input type="text" name="modelo" value="${v.modelo||''}" placeholder="Ex: Mercedes Benz 1621" style="${IS}" ${FO}>
            </div>

            <div>
                ${LB('calendar-alt','Ano')}
                <input type="text" name="ano" value="${v.ano||''}" placeholder="2015" maxlength="4" style="${IS}" ${FO}>
            </div>

            <div>
                ${LB('palette','Cor')}
                <input type="text" name="cor" value="${v.cor||''}" placeholder="Ex: Branco" style="${IS}" ${FO}>
            </div>

            <div style="grid-column:span 2;">
                ${LB('sticky-note','Observações')}
                <textarea name="observacoes" rows="3" placeholder="Capacidade, características, documentação..."
                    style="${IS}resize:vertical;" ${FO}>${v.observacoes||''}</textarea>
            </div>
        </div>
        </div>
        ...
        <div style="display:flex;justify-content:flex-end;gap:10px;padding-top:16px;border-top:1px solid #f1f5f9;">
            <button type="button" onclick="Pages._fecharModalVeiculo()"
                style="padding:10px 22px;border-radius:10px;border:1.5px solid #e2e8f0;background:#fff;color:#64748b;font-size:13px;font-weight:600;cursor:pointer;">
                Cancelar
            </button>
            <button type="submit"
                style="padding:10px 28px;border-radius:10px;border:none;background:#c9a227;color:#fff;font-size:13px;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:8px;">
